      ...
      <div className="max-w-4xl mx-auto text-center">
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-12 md:gap-20 mb-24">
            <a href="mailto:hello@kharisma.life" className="group flex flex-col items-center gap-4 transition-transform hover:-translate-y-1">
            ...
            {config?.footer.links.find(l => l.label === 'Instagram') && (
              <a href={config.footer.links.find(l => l.label === 'Instagram')?.url} className="group flex flex-col items-center gap-4 transition-transform hover:-translate-y-1">
                <div className="w-20 h-20 rounded-full border border-natural-border flex items-center justify-center group-hover:bg-natural-olive group-hover:text-black transition-all duration-500 shadow-sm text-white">
            ...
            {config?.footer.links.find(l => l.label === 'YouTube') && (
              <a href={config.footer.links.find(l => l.label === 'YouTube')?.url} className="group flex flex-col items-center gap-4 transition-transform hover:-translate-y-1">
                <div className="w-20 h-20 rounded-full border border-natural-border flex items-center justify-center group-hover:bg-natural-olive group-hover:text-black transition-all duration-500 shadow-sm text-white">
          ...
          <div className="pt-24 border-t border-natural-border flex flex-col items-center gap-6 text-[10px] uppercase tracking-[0.2em] text-natural-muted font-bold">
            <p>{config?.footer.text || `© ${new Date().getFullYear()} — kharisma.life. All Rights Reserved.`}</p>
          </div>
        </motion.div>
      </div>
    </section>
